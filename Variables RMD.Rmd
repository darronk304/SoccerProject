...
`SH`: Shots total the does not include penalty kicks

`SoT`: Shots on target that do not include penalty kicks

`SoT_percent`: Percentage of shots that are on target. Shots on target do not include penalty kicks
...
`Comp`: Annual compensation averaged across selected seasons
...
`Press_Pressures`: Number of times applying pressure to opposing player who is receiving, carrying or releasing the bal
...
`Final_Third_Carries`: Carries that enter the 1/3 of the pitch closest to the goal
...
`Dis_Carries`: Number of times a player loses control of the ball after being tackled by an opposing player. Does not include attempted dribbles
